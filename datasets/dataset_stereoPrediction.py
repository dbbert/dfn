...
class DataHandler(object):
  ...
  def __init__(self, image_size=64, binarize=False, num_frames=2, batch_size=2, mode='train'):
    if mode == 'train':
    ...
    self.data_L_ = f['highway_L'].value.transpose(0,1,3,2)
    self.data_R_ = f['highway_R'].value.transpose(0,1,3,2)
    self.dataset_size_ = self.data_L_.shape[0]
    self.num_channels_ = self.data_L_.shape[1]
    self.image_size_ = self.data_L_.shape[2]
  ...
  def GetBatch(self, ind=None):
    if ind is None:
      ...
      ind = np.random.choice(self.dataset_size_, batch_size)
    else:
      batch_size = len(ind)
    ...
    for j in xrange(batch_size):
      data[j, :, :, :, 0] = self.data_L_[ind[j], :, :, :]
      data[j, :, :, :, 1] = self.data_R_[ind[j], :, :, :]
