...
class Model(object):
    ...
    def predict(self, input, hidden_state, Ws, bs):
        ...
        ###############################
        ## encoder
        output = ConvLayer(input, num_filters=32, filter_size=(3,3), stride=(1,1), pad='same', W=Ws[f], b=bs[f], nonlinearity=leaky_rectify, untie_biases=True); Ws[f] = output.W; bs[f] = output.b; f = f+1
        ...
        hidden = ConvLayer(hidden_state, num_filters=64, filter_size=(3,3), stride=(1,1), pad='same', W=Ws[f], b=bs[f], nonlinearity=leaky_rectify); Ws[f] = hidden.W; bs[f] = hidden.b; f = f+1
        hidden = ConvLayer(hidden, num_filters=64, filter_size=(3, 3), stride=(1,1), pad='same', W=Ws[f], b=bs[f], nonlinearity=leaky_rectify); Ws[f] = hidden.W; bs[f] = hidden.b; f = f+1
